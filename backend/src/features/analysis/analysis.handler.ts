...
import {
  analysisPairParamsSchema,
  analysisResultsQuerySchema,
  analyzeQuerySchema,
...
export function analysisResultsHandler(c: Context) {
  const parsed = analysisResultsQuerySchema.safeParse({
    threshold: c.req.query("threshold") || "0",
  });
  if (!parsed.success)
    throw new AppError("Threshold tidak valid", 400, parsed.error.flatten());
...
export function analysisPairHandler(c: Context) {
  ...
  if (!parsed.success)
    throw new AppError(
      "Parameter pasangan tidak valid",
      400,
      parsed.error.flatten(),
    );
