...
import { findAnalysisFileById, findAnalysisPairResult, getAnalysisSettings, listAnalysisFiles, listAnalysisResults, saveAnalysisResults } from "./analysis.repository";
...
export function checkMetadataSuspicion(metadata: Record<string, string>): { status: string; reason: string } {
  ...
  const authorList = authors ? authors.split(",").map((item) => item.trim()) : [author];
...
export function getFileRanking(files: ParsedFile[], results: SimilarityResult[]): RankedFile[] {
  const fileMap = new Map<string, { similarities: number[]; metadata: Record<string, string>; name: string }>();

  for (const file of files) {
    fileMap.set(file.id, { similarities: [], metadata: file.metadata, name: file.name });
  }
...
export function detectDirection(fileA: ParsedFile, fileB: ParsedFile, result?: SimilarityResult): string {
...
export function runAnalysis(courseName: string) {
  ...
  if (files.length < 2) throw new AppError("Minimal 2 file untuk analisis", 400);
...
export function getAnalysisResults(threshold: number) {
  ...
  return {
    files,
    results: threshold > 0 ? results.filter((result) => result.normalScore >= threshold) : results,
    settings,
  };
}

export function getRanking() {
  return { ranking: getFileRanking(listAnalysisFiles(), listAnalysisResults()) };
}
...
export function getPairDetail(idA: string, idB: string): PairDetailResponse {
  ...
  return {
    fileA: { ...fileA, metadataStatus: fileAMetadata.status, metadataReason: fileAMetadata.reason },
    fileB: { ...fileB, metadataStatus: fileBMetadata.status, metadataReason: fileBMetadata.reason },
    similarity: similarity || { normalScore: 0, strictScore: 0, wordOverlap: 0, matchedRanges: [] },
